...
export function RepositoryDetail() {
  ...
  async function start(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = event.currentTarget; const prompt = new FormData(form).get('prompt')?.toString().trim(); if (!prompt) return;
    setStarting(true);
    try { const created = await api.startTask(id, prompt); tasks.setData([created, ...(tasks.data ?? [])]); setSelectedTask(created.id); setStartError(''); form.reset(); }
