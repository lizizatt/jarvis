...
import { ArrowLeft, GitCompare, History, MessageSquare, Monitor, Play, Plus, TerminalSquare } from 'lucide-react';
...
export function RepositoryDetail() {
  ...
  return <main className="detail-page" data-testid="repository-detail">
    <AmbientSigil />
    <header className="detail-header"><Link to="/" className="icon-button" aria-label="Back to repositories"><ArrowLeft /></Link><div><h1>{repo.name}</h1><p>{repo.status?.branch ?? repo.defaultBranch}</p></div><button className="icon-button" aria-label="New task" onClick={() => { setTab('agent'); setSelectedTask(undefined); }} data-testid="header-new-task"><Plus /></button><Link className="icon-button" aria-label="Manage history" to={`/repositories/${id}/history`} data-testid="manage-history"><History /></Link></header>
