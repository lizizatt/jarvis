...
This is the execution checklist for the [verified findings](DEPLOYABILITY_FINDINGS.md). The review is historical evidence; this document tracks implementation and validation. The user subsequently approved branch creation and commits, but not pushing or deployment. Work is paused; see the [handoff](DEPLOYABILITY_HANDOFF.md) for the current continuation point.
...
6. [x] **Establish the real-systemd release gate.** Document/provide a disposable-VM procedure for fresh install, upgrade, failure recovery, rename/removal, slow actions, reboot, and uninstall. The [VM procedure](../VM_RELEASE_GATE.md) and one-off harness are established. Execution reached six passing phases before a reboot failure; a retry was cancelled. Completion remains outstanding.
7. [x] **Validate and adversarially review.** Run focused tests, lint/typecheck, full tests and build in isolation where possible. Review implementation against every finding, update operator documentation, and record unavailable checks. Isolated checks, container validation, and scoped browser checks passed. The VM gate is incomplete; extension-host/real Copilot release capability remains unestablished. Fixes are not live-deployment-verified.
...
- Explicit rename/removal retires only previously recorded managed units and keeps recovery inventory until retirement completes. Uninstall consumes v2 inventory without source manifests. Fixtures cover both; real-systemd rename/removal passed, but post-reboot uninstall remains unverified.
...
- Step 6 now includes disposable-VM execution: preflight, fresh core, synthetic registry migration, failure recovery, rename/removal, and slow-action phases passed. Reboot failed and uninstall was not reached. The retained readiness fix is locally tested but awaits a completed VM run. See the [execution record](DEPLOYABILITY_GATE_EXECUTION.md).
...
- Regression checks cover both immediate activation failure and readiness failure after a replacement starts, including retry. Baseline source was committed as `4645c89` on `feat/deployment-hardening`; subsequent harness changes are retained with the handoff. Nothing was deployed.

## Next release gate

When work resumes, address cancellation cleanup and complete the disposable-VM run described in the handoff. Container and scoped browser evidence already exist. A real historical-version upgrade, VS Code/Copilot capability, and broader phone/offline coverage remain separate checks. Versioned application releases and globally atomic activation are not implemented by this change.
